...
export const GrandfatherCard = ({ iconStyle }: GrandfatherCardProps) => {
  ...
  useEffect(() => {
    setReloadPage(false);
    const storedValue = localStorage.getItem("Show-grandfather-Card");
    if (storedValue !== null) {
      setShowGrandFatherCard(JSON.parse(storedValue));
    };

    // fetching detail of card
    const details = localStorage.getItem("Grand-Father");
    if (details) {
  ...
  useEffect(() => {
    if (!reloadPage) {
      ...
      localStorage.setItem("Grand-Father", JSON.stringify(cardDetail));
      localStorage.setItem("Show-grandfather-Card", JSON.stringify(showGrandFatherCard));
    }
